...
impl FastMap {
    ...
    pub fn keys<'a>(&'a self) -> impl Iterator<Item=u8> + 'a {
        (0..self.data.len()).filter(move |i| self.data[*i].is_some()).map(|i| i as u8)
    }
...
impl Extend<(u8, u8)> for FastMap {
    ...
    where T: IntoIterator<Item=(u8,u8)> {
        for (k,v) in iter {
            self.insert(k,v);
        }
    }
...
impl FromIterator<(u8,u8)> for FastMap {
    ...
    where I:IntoIterator<Item=(u8,u8)>
    {
